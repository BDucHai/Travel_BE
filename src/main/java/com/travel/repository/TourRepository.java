...
public interface TourRepository extends JpaRepository<Tour, Long> {
  ...
  @Query(value = """
      ...
      FROM Tour t
      JOIN t.styles s
      WHERE (s.slugEn = :slug OR s.slugFr = :slug)
        AND t.status = 'PUBLISHED'
        AND t.isActive = true
      ORDER BY t.createdAt DESC
      """, countQuery = """
      ...
      WHERE (s.slugEn = :slug OR s.slugFr = :slug)
        ...
        AND t.isActive = true
      """)
  Page<Tour> findPublishedByStyleSlugPage(
  ...
  @Query(value = """
      ...
      FROM Tour t
      JOIN t.collections c
      WHERE (c.slugEn = :slug OR c.slugFr = :slug)
        AND t.status = 'PUBLISHED'
        AND t.isActive = true
      ORDER BY t.createdAt DESC
      """, countQuery = """
      ...
      WHERE (c.slugEn = :slug OR c.slugFr = :slug)
        ...
        AND t.isActive = true
      """)
  Page<Tour> findPublishedByCollectionSlugPage(
      @Param("slug") String slug,
      Pageable pageable);
  ...
  @Query(value = """
      ...
      WHERE (d.slugEn = :slug OR d.slugFr = :slug)
        AND t.status = 'PUBLISHED'
        AND t.isActive = true
      ORDER BY t.createdAt DESC
      """, countQuery = """
      ...
      WHERE (d.slugEn = :slug OR d.slugFr = :slug)
        ...
        AND t.isActive = true
      """)
  Page<Tour> findPublishedByDestinationSlugPage(
      @Param("slug") String slug,
      Pageable pageable);
  ...
  @Query("""
      SELECT t
      FROM Tour t
      WHERE LOWER(t.titleEn) LIKE LOWER(CONCAT('%', :keyword, '%'))
      AND t.status = 'PUBLISHED'
      AND t.isActive = true
      ORDER BY t.createdAt DESC
      """)
  Page<Tour> searchByTitleEn(
  ...
  @Query("""
      SELECT t
      FROM Tour t
      WHERE LOWER(t.titleFr) LIKE LOWER(CONCAT('%', :keyword, '%'))
      AND t.status = 'PUBLISHED'
      AND t.isActive = true
      ORDER BY t.createdAt DESC
      """)
  Page<Tour> searchByTitleFr(
      @Param("keyword") String keyword,
      Pageable pageable);
}